"""ResNet-38 backbone for the Stage-1 image-level classifier.

Architecture: ResNet-38 (Atrous) with three feature taps (b4_5, b5_2,
final) that feed the CAM branches in ``resnet38_cls.py``.

Provenance note: the feature-refinement modules in this file (the
squeeze-excitation blocks and the edge/semantic fusion block) originate
from the authors' previous work. They are part of the *standard* CAM
pipeline used only to produce pseudo masks and are NOT a contribution
of this paper - Stage 1 is described in the manuscript as a conventional
CAM-based pipeline. They are kept unchanged so that the published
pseudo masks remain reproducible.

Pretrained weights: ImageNet weights in MXNet format
(``ilsvrc-cls_rna-a1_cls1000_ep-0001.params``) can be converted with
``convert_mxnet_to_torch``. ``mxnet`` is only needed for this one-off
conversion.
"""
...
class SEBlock(nn.Module):
    """Lightweight squeeze-excitation channel-attention block."""
    ...
    def forward(self, x):
        # Global average pooling -> 1x1 conv (reduce) -> ReLU
        # -> 1x1 conv (restore) -> Sigmoid, then reweight the input.
        y = self.avg_pool(x)
        y = self.fc(y)
        return x * y
...
class EdgeSemanticFusion(nn.Module):
    """Edge/semantic feature-refinement block (see provenance note above).

    Combines a fixed Laplacian edge response, channel-wise semantic
    weights and spatial-pyramid context. Kept identical to the original
    experiment code so the Stage-1 pseudo masks remain reproducible.

    The original code named this module ``SMFE``; it is renamed here to
    avoid confusion with a component of the authors' previous paper.
    """

    def __init__(self, in_channels, reduction=16):
        super(EdgeSemanticFusion, self).__init__()
    ...
    def forward(self, x):
        ...
        guidance = self.guidance_weight(x)
...
class Net(nn.Module):
    def __init__(self):
        ...
        # Channel attention on the b4_5 tap (shallow-level semantics).
        self.se_b4 = SEBlock(512, reduction=16)
        ...
        # Channel attention on the b5_2 tap.
        self.se_b5 = SEBlock(1024, reduction=16)

        self.b6 = ResBlock_bot(1024, 2048, stride=1, dilation=4, dropout=0.3)

        self.feature_fusion = EdgeSemanticFusion(2048)
    ...
    def forward_as_dict(self, x):
        ...
        # Apply channel attention after b4_5.
        b_45 = self.se_b4(b_45)
        ...
        # Apply channel attention after b5_2.
        b_52 = self.se_b5(b_52)
