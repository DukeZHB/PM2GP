"""Inference script of the legacy PSPNet baseline.

This is the baseline segmentation model (PSPNet with a timm-resnest200e
encoder, via ``segmentation_models_pytorch``) used for comparison in the
experiments. It is NOT the PM2GP model - for the diffusion-initialized
segmentation network see ``segmentation/``. Preserved for baseline
reproduction.

Note: like the original baseline, this script applies a heuristic
white-background mask after argmax (pixels brighter than a gray
threshold are forced to the background class). This post-processing is
part of the baseline only.
"""
...
from legacy import seg_transformers as tr
...
def main():
    parser = argparse.ArgumentParser(description="Legacy PSPNet baseline inference")
    parser.add_argument('--dataset', type=str, default='luad')
    parser.add_argument('--n_class', type=int, default=4)
    parser.add_argument('--data-dir', type=str, default='./datasets/LUAD-HistoSeg/train',
                        help='Folder containing an img/ subdirectory.')
    parser.add_argument('--checkpoint', type=str, required=True,
                        help='Path to the PSPNet checkpoint (.pth).')
